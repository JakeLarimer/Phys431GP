...
df = pd.read_csv("GCS.csv", na_values=["nd"])

# load GC color data into another dataframe
df_color = pd.read_csv('GC_color.csv')

# add GCS data to galaxies we have color info on
# df_combined = pd.merge(df, df_color, left_on='Galaxy', right_on='Galaxy Name', how='inner')

# OR
df_combined = pd.read_csv("GC_combined.csv")
